...
const config = {
  EMAIL: 'd@d.com',
  PASS: '11111111',
  DEVICE_ID: 'hauBcqChBiHAaK1Gb5ON',
  pipe: 'pipe.json',
  firebaseConfig: {
      ...
      fs.readFile(config.pipe, (err, data) => {
        ...
        if (data.toString().trim()) {
          try {
            ...
            for (let key in tempJSON.settings) {
              if (!isEqual(tempJSON.settings[key], tempData['settings'][key])) {
                hasChanges = true;
                updateObject[`settings.${key}`] = tempJSON.settings[key];
              }
            }
            for (let key in tempJSON.details) {
              if (!isEqual(tempJSON.details[key], tempData['details'][key])) {
                hasChanges = true;
                updateObject[`details.${key}`] = tempJSON.details[key];
              }
            }
